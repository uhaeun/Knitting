...
/**
 * 웹 전용 결과 사진 합성 (Canvas 2D). 앱은 composeResult.ts (Skia).
 * 결과는 blob URL. 저장은 파일 내려받기, 공유는 Web Share.
 */
...
export async function composeResult(scene: Scene, _fileName: string): Promise<string> {
...
export async function saveResult(uri: string, fileName: string): Promise<void> {
...
export async function shareResult(uri: string, fileName: string): Promise<void> {
