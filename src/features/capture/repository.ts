...
export async function savePost(input: {
  ...
  try {
    getDb().runSync(
      `INSERT INTO posts (id, project_id, photo_path, thumb_path, width, height, taken_at, created_at, updated_at, deleted_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)`,
      post.id, post.project_id, post.photo_path, post.thumb_path,
      post.width, post.height, post.taken_at, post.created_at, post.updated_at,
    );
  } catch (e) {
    deleteIfExists(photoRel);
...
export function deletePost(id: string): void {
  const now = new Date().toISOString();
  getDb().runSync('UPDATE posts SET deleted_at = ?, updated_at = ? WHERE id = ?', now, now, id);
}
