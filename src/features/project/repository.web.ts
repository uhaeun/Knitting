...
import { requireUserId, signPaths } from '@/shared/lib/remote.web';
...
/**
 * 웹 전용 편물 저장소. repository.ts와 함수 이름이 같고 결과는 Promise다
 * (queries.ts가 Promise.resolve로 감싸므로 그대로 동작한다).
 * ProjectSummary.cover_thumb_path에는 Storage 키 대신 서명 URL이 들어간다 → files.web.ts의 toUri는 그대로 돌려준다.
 */
...
/** 웹은 서버가 원본이라 올릴 것이 없다 */
export const unsyncedCount = (): number => 0;
