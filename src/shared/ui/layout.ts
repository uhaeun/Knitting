...
/**
 * 화면 콘텐츠 너비. 넓은 창(데스크톱·VS Code)에서도 폰 너비 한 칸(size.webColumn)을 넘지 않는다.
 * useWindowDimensions().width를 사진 크기 계산에 직접 쓰지 않고 이 훅을 쓴다.
 */
export function useContentWidth(): number {
  const { width } = useWindowDimensions();
  return Math.min(width, size.webColumn);
}

/** 정사각 사진의 한 변. 창이 낮으면 버튼이 밀려나지 않게 높이에서 사진 외 공간(chrome)을 뺀 만큼으로 줄인다. */
