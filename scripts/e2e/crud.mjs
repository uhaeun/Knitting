...
try {
  ...
  await page.getByText('바꾼 이름').first().waitFor({ timeout: 15000 });
  ...
  await choose('지우기');
  await page.getByText('2번째 / 2').waitFor({ timeout: 15000 });
  check('3장 → 2장', true);

  console.log('\n== 게시물 상세에서 내 기록 지우기 (D)');
  await page.goto(`${BASE}/me`);
  const cells = page.locator('[role="button"]:has(img)');
  await cells.first().waitFor({ timeout: 30000 });
  ...
  await page.waitForFunction(() => document.querySelectorAll('[role="button"] img').length === 1, null, { timeout: 15000 }).catch(() => {});
  check('나 탭에 1장', (await cells.count()) === 1, `(${await cells.count()})`);

  console.log('\n== 목록에도 반영');
  await page.goto(`${BASE}/projects`);
  await page.getByText('바꾼 이름').first().waitFor({ timeout: 30000 });
