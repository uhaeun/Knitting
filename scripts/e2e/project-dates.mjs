...
try {
  ...
  await page.getByText('4월 1일 시작', { exact: false }).last().waitFor({ timeout: 60000 });
  ...
  await page.getByRole('button', { name: '저장', exact: true }).click();
  await page.getByText('완성', { exact: false }).last().waitFor({ timeout: 60000 });
  check('완성일이 채워짐', !row().endsWith('null'), row());

  await page.goto(`${BASE}/projects`);
  await page.getByText('완성', { exact: true }).first().waitFor({ timeout: 60000 });
